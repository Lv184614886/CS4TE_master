    ...
    def __init__(self, config, prefix, is_test, tokenizer):
        ...
        if self.config.debug:
            ...
            self.json_data = json.load(
                open(os.path.join(self.config.data_path, prefix + '.json'), 'r', encoding='utf-8'))
    ...
    def __getitem__(self, idx):
        ...
        if not self.is_test:
            ...
            if s2ro_map:
                ...
                return token_ids, masks, loss_masks, text_len, triple_matrix, ins_json_data[
                    'triple_list'], tokens, self.config.rel_num
            else:
                return None
...
def re_collate_fn(batch):
    ...
    # rel_num,if use WebNLG_star, modify 24 to 171
    rel_num = rel_nums[0]
